...
public class ShowApprovedCourierOffersCommand implements ActionCommand {
    ...
    public String execute(HttpServletRequest request) throws CommandException {
        try {
            logger.info("ShowApprovedCourierOffersCommand executed");
            String page = Path.APPROVED_COURIER_OFFERS_PATH;
            request.setAttribute(AttributeName.OFFER_MAP, CourierOfferLogic.INSTANCE.findApprovedCourierOffers(Integer.parseInt(request.getSession().getAttribute(ParamName.ID_USER_PARAM).toString())));
